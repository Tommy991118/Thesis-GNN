...
# Adjust below values to fit the amount of nodes of a POI in graph.
...
class DataUtils:
    def __init__(self, graph_file, is_all=False, node_negative_distribution_temp = None, test_indices = None):
        ...
        with np.load(graph_file, allow_pickle=True) as loader:
            ...
            # The code below creates feature matrices of the different node types of the loaded in graph files.
            # It is saved on the level of the dataloader object.
            if loader['attr_data1'].all() != None:
                print("Attributes Found.")
                #Need to also save attribute data. So in essence to have attributes of the indices, indptr, etc.  
    ...
    #The following method returns a dictionary that maps node names to their corresponding embeddings.
    #Allows to look up node embeddings by node name?
