...
def train_test_split_sampled(arrName, pivotColumn, num_of_intervals):
    ...
    #checks if any element on an axis, in this case 1, gives True
    ...
    while(right < len(bins)):
      ...
      right = right + 1
      portion_arr = []
# Basically manual bins are made, out of each bin a test and train split is taken and added to the total test and train data, we think because they binned on 'price' and then wanted a stratified test and train split
...
def score_node_classification(features, z, features_test, labels_test, p_labeled=0.8, n_repeat=10, norm=False):
    ...
    for seed in range(n_repeat):
        ...
        #Fitting it on all data and taking the original split (previous one did an extra split)
        rfr = RandomForestRegressor(n_jobs=-1)
        rfr.fit(features, z)
...
def check_performance(number_of_iter, test_indices):
    ...
    #Select if ID of the csv is in the test set or not 
...
#initializes variables
        ...
        for b in range(args.num_batches):
            ...
            #Takes start time
            t1 = time.time()
            #CREATE DIFFERENT SAMPLER, Not in every batch doing the same calculations, batch 1 does house-train, batch 2 does house school etc
            if b%5 < 0:
